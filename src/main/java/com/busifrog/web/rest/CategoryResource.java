...
import com.busifrog.repository.CategoryRepository;
...
/**
 * REST controller for managing {@link com.busifrog.domain.Category}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CategoryResource {
        ...
        Optional<Category> result = categoryRepository
            ...
            .map(
                existingCategory -> {
                    ...
                    if (category.getIsApproved() != null) {
                        existingCategory.setIsApproved(category.getIsApproved());
                    }
    ...
    public ResponseEntity<List<Category>> getSubCategories(@PathVariable Long id, Pageable pageable) {
        log.debug("REST request to get Category : {}", id);
        Page<Category> page = categoryRepository.findAllSubCategoriesById(id, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
